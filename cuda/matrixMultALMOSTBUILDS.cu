...
__device__ void correlationCoefficient(int **l_win, int **r_win, int mtx_size, double *out)
{
    // calc L dot 1
    double Ld1;
    sumVect<<<1, mtx_size>>>(l_win, mtx_size, &Ld1);
...
__global__ void matrixMult(int *a, int *b, int *out, int mtx_size)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;

    int outIndex = x + y * mtx_size;
    ...
    {
        int aInd = i + (y * mtx_size);
        int bInd = x + (i * mtx_size);
